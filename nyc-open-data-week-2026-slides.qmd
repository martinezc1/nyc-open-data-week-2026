...
-   [Crystal Adote](https://www.linkedin.com/in/crystal-adote-0305641ba/)
-   [Jonah Dratfield](https://www.linkedin.com/in/jonah-dratfield-932584167/)
-   [Joyce Escatel Flores](https://www.linkedin.com/in/joyce-escatel-flores/)
-   [Robert Hutto](https://www.linkedin.com/in/robert-hutto-2497ab181/)
-   [Isley Jean-Pierre](https://www.linkedin.com/in/isley-jean-pierre-21aa00396/)
-   [Shannon Joyce](https://www.linkedin.com/in/shannonjoyce222/)
-   [Emma Valentina Tupone](https://www.linkedin.com/in/emma-tupone-1353a1291/)
-   [Xinru Wang](https://www.linkedin.com/in/xinru-wang-798937393/)
-   Laura Werner
...
snap_plot <- snap_plot_data %>%
  ...
  theme_minimal()
library(patchwork)
event_plot + plot_spacer() + snap_plot +
...
::: {.column width="42%"}
-   The Knicks rank **23rd** out of 32 teams in home-court scoring advantage
-   Bottom 1/3 of the league.
-   MSG as a home-court may actually be more of a detriment than a benefit.
:::
:::::
...
### Examining How Juvenile Probation And School Discharge Contribute To Recidivism
...
-   This project investigates:
    -   Rearrest Rates, Probation Caseloads, and School Discharge Patterns
...
-   **Cramer's V (0.54)**. Moderate to strong relationship between discharge category and school level.
...
t_test_discharge
wilcox.test(discharge_rate ~ school_level, data = sdis_male)
```
...
## School Discharges By Districts
...
-   [LinkedIn](www.linkedin.com/in/shannonjoyce222%22)
...
-   Mold Exposure —\> Psychological Stress/Aggression

-   Psychological Stress/Aggression —\> Domestic Violence

[**Do domestic violence reports and residential mold complaints in NYC follow similar, correlated patterns over time?**]{style="color: #994C99;"}

## Datasets: Mold Complaints

[311 Service Requests](https://data.cityofnewyork.us/Social-Services/311-Service-Requests-from-2020-to-Present/erm2-nwe9/about_data)

```{r}
#| label: mold datasets
#| echo: false
#| tbl-cap: "Table 1. Aggregated residential mold complaints by borough, 2010-2024."
head(aggregated_mold_data, n=5) %>% kable()
```

## Datasets: Domestic Violence Reports
...
#| label: dv datasets
#| echo: false
#| tbl-cap: "Table 2. Aggregated domestic violence reports by borough, 2010-2024."
head(aggregated_dv_data, n=5) %>% kable()
```
...
#| label: heatmaps
#| echo: false
#| fig-cap: "Figure 1. DV reports & mold complaints by borough and year, 2010-2024. Darker colors represent a higher volume of complaints/reports."
...
#| label: corrplot1
#| echo: false
#| fig-cap: "Figure 2. A scatterplot representing a positive correlation between total mold complaints and DV reports, grouped by borough."
...
#| label: trends
#| echo: false
#| fig-cap: "Figure 3. Line plots representing monthly DV reports & mold complaints by borough, 2010-2024."
...
## Correlation Test: DV Reports \~ Mold Complaints (Month to Month)
...
-   [**r = 0.36**]{style="color: #994C99;"}
-   [***p*** **\< 0.001**]{style="color: #994C99;"}
...
#| label: corrplot2
#| echo: false
#| fig-cap: "Figure 4. A scatterplot representing a positive correlation between current-month mold complaints and DV reports 3 months later."
dv_mold_lagged <- aggregated_dv_mold_data %>%
  arrange(Borough, time_index) %>%
  group_by(Borough) %>%
  mutate(DV_3_month = lead(`DV Reports`, n = 3)) %>%
  ungroup()

ggplot(dv_mold_lagged, aes(x = `Mold Complaints`, y = DV_3_month)) +
  geom_point(alpha = 0.5, color = "darkgreen") +
  geom_smooth(method = "lm", color = "darkred") +
  labs(
    title = "Current Month Mold Complaints vs. DV Reports Three Months Later",
    x = "Mold Complaints (Current Month)",
    y = "DV Reports (Three Months Later)"
  ) +
  theme_minimal()
```
...
#| echo: false
#| output: hide
borough_results <- dv_mold_lagged %>%
     group_by(Borough) %>%
     dplyr::summarize(
         lag_cor = cor(`Mold Complaints`, DV_3_month, use = "complete.obs")
     )
```

### Current Month Mold Complaints \~ DV Reports 3 Months Later

-   [**r = 0.25**]{style="color: #994C99;"}
-   [***p*** **\< 0.001**]{style="color: #994C99;"}
...
| Mold \~ DV + Borough + [<strong>Avg. Resolution Days</strong>]{style="color:#994C99;"} | 0.95 | \< 0.001 | [<strong>10524.65</strong>]{style="color:#994C99;"} |
| Mold \~ [<strong>DV (3 Months Later)</strong>]{style="color:#994C99;"} + Borough | 0.94 | \< 0.001 | [<strong>10390.65</strong>]{style="color:#994C99;"} |

## Key Findings & Takeaways {#sec-key-findings--takeaways}

-   Housing distress 🔗 Domestic instability
-   Predictive power of [**both models**]{style="color: #994C99;"}
    -   Response speed
    -   3-month lag
-   [**Proactive**]{style="color: #994C99;"} roadmap for NYC agencies
    -   NYC Housing Authority
    -   NYC Department of Health & Mental Hygiene

[**Full Project: *https://rpubs.com/shannonjoyce/toxichomes***]{style="color: #994C99;"}
...
### Beating Around the Bush: Urban Trees and Wildlife Patterns in New York City
...
### Domestic Violence Incidents vs Resource Allocation Across NYC
...
-   [Family Violence Related Snapshots](https://data.cityofnewyork.us/Public-Safety/Family-Violence-Related-Snapshots-New-York-City-Co/a35y-93e7/about_data)

    -   Domestic violence–related incident reports by community district and borough

-   [Annual Report on Domestic Violence Initiatives](https://data.cityofnewyork.us/Social-Services/Annual-Report-on-Domestic-Violence-Initiatives-Ind/7t9i-jsfp/about_data)
...
```{r}
#| label: fig-services_borough_cat_2020
#| message: false
#| warning: false
